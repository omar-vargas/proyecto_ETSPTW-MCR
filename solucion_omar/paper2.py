...
def apply_simulated_annealing(current_solution, nodes, distance_matrix, evaluation_criterion , alpha:int = 0.9995, temp_init:int = 100):

    best_solution = current_solution
    best_feasible, best_energy = evaluation_criterion(current_solution,  nodes, distance_matrix)
    ...
    while temperature > 1e-8 or not best_feasible:
        ...
        if operator == 'shift':
            new_solution = apply_1shift(current_solution,  nodes, distance_matrix)
        elif operator == '2opt':
            new_solution = apply_2opt(current_solution,  nodes, distance_matrix)
        elif operator == 'swap':
            new_solution = apply_swap(current_solution,  nodes, distance_matrix)
        elif operator == 'shuffle':
            new_solution = apply_shuffle(current_solution)

        
        feasible, new_energy = evaluation_criterion(new_solution, nodes, distance_matrix)

        if feasible:
            current_energy = evaluation_criterion(current_solution, nodes, distance_matrix)[1]
...
def hybrid_sa_ts(
    ...
    solution_X = apply_simulated_annealing(solution_X, nodes, distance_matrix, is_time_feasible)
